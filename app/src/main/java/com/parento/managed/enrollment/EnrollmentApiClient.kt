...
class HttpEnrollmentApiClient(
    ...
    override suspend fun consume(
        ...
        runCatching {
            val endpoint = URL(baseUrl.trimEnd('/') + "/api/v1/devices/enrollments/" + authorization.enrollmentId + "/consume")
            ...
            try {
                ...
                if (status in 200..299) parseSuccess(responseBody) else mapFailure(status, responseBody)
            } finally {
    ...
    private fun parseSuccess(body: String): OperationResult<EnrollmentResult> =
        runCatching {
            ...
            val deviceCredential = data.getString("deviceCredential").trim()
            val enrollmentId = enrollment.getString("id").trim()
            require(managedDeviceId.isNotBlank() && isUuid(enrollmentId) && isToken(deviceCredential))
            EnrollmentResult(
                managedDeviceId = managedDeviceId,
                enrollmentId = enrollmentId,
                expiresAtEpochMillis = java.time.Instant.parse(enrollment.getString("expiresAt")).toEpochMilli(),
    ...
    private fun mapFailure(status: Int, body: String): OperationResult<EnrollmentResult> {
        ...
        return when {
            status == 401 || status == 403 -> OperationResult.Failure(ManagedError.AUTHORIZATION_FAILURE)
            status == 404 && code == "ENROLLMENT_NOT_FOUND" -> OperationResult.Failure(ManagedError.INVALID_STATE)
            status == 409 || status == 410 -> OperationResult.Failure(ManagedError.INVALID_STATE)
            status == 429 || code == "RATE_LIMITED" -> OperationResult.Failure(ManagedError.RATE_LIMITED)
            status >= 500 || status == 408 -> OperationResult.Failure(ManagedError.NETWORK_FAILURE)
            status == 400 -> OperationResult.Failure(ManagedError.INVALID_STATE)
            else -> OperationResult.Failure(ManagedError.UNKNOWN)
        }
    }
    ...
    private fun readBounded(reader: java.io.Reader): String {
        ...
        while (true) {
            ...
            if (count == -1) break
            total += count
            if (total > MAX_RESPONSE_CHARS) throw IOException("Enrollment response exceeded the allowed size.")
